...
<FORM ACTION=Filtros.php METHOD=post>
    ...
    <ul class="navbar-nav mr-auto">
            <li class = "nav-item">
                <button class="volver-nav-btn" type ="button" onclick="location.href = 'Busquedas.php'">Volver</button><br><br>
            </li>
        </ul>    </nav>
    <div class="mx-auto-subtitle">
        <h2>Busqueda por Calificación</h2>
    </div>
    <div class="mx-auto-lista1">
        <ul class = "lista2">
            <li>
                <label class = 'input-text-busqueda'>Calificacion Mayor Que:</label> <INPUT class= "input-tb-busqueda" TYPE=text NAME=CALIFICACION required><BR>
            </li>
            <li>
                <INPUT class="input-btn-filtro" TYPE=submit NAME=OK VALUE="Filtrar"><BR>
            </li>
        </ul>
    </div>
</FORM>
...
if (isset($OK)) {
    if ($OK == "Filtrar") {
        ...
        $q = "select * from talumnos where calificacion >= " . $CALIFICACION;
